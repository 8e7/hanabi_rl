...
class R2D2Agent(torch.nn.Module):
    ...
    def act(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.save_path is not None:
            #self.obs_list = torch.cat([self.obs_list, obs['priv_s']])
            #self.obs_cnt += 1
            #torch.save(reply, self.save_path + f"/reply_{self.obs_cnt}.pt")
            self.append_observation(obs, reply)
        return reply

    def append_observation(self, obs: Dict[str, torch.Tensor], reply: Dict[str, torch.Tensor]):
    ...
    def write_episode(self, episode_num):
        obs_numpy = [{k: v.numpy() for k, v in d.items()} for d in self.obs_list]
        reply_numpy = [{k: v.numpy() for k, v in d.items()} for d in self.reply_list]
        np.save(self.save_path + f"/obs_{episode_num}.npy", obs_numpy, allow_pickle=True)
        np.save(self.save_path + f"/reply_{episode_num}.npy", reply_numpy, allow_pickle=True)

        self.obs_list = torch.jit.annotate(List[Dict[str, torch.Tensor]], [])
        self.reply_list = torch.jit.annotate(List[Dict[str, torch.Tensor]], [])
