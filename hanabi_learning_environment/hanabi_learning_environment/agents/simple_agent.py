...
env_config = {
    ...
    "episodes_file":            "./episodes/SimpleAgent/episodes.txt"
}
...
def evaluate(model_list, env, eval_num=100, vis=False):
    ...
    for iteration in range(eval_num):
        ...
        while not done:
            ...
            observation = env.unwrapped.state.observation(player)
            action, _state = model_list[player].predict(observation, deterministic=True)
...
if __name__ == "__main__":
    ...
    evaluate([SimpleAgent, SimpleAgent], env, eval_num=10, vis=False)
